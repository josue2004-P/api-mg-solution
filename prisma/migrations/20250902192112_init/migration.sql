...
CREATE TABLE `whatsApp_configuration` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `initial_message` VARCHAR(100) NOT NULL,
...
-- CreateTable
CREATE TABLE `galery_configuration` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `title` VARCHAR(50) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `product` ADD CONSTRAINT `product_brandId_fkey` FOREIGN KEY (`brandId`) REFERENCES `product_brand`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `product` ADD CONSTRAINT `product_userCreateId_fkey` FOREIGN KEY (`userCreateId`) REFERENCES `users`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
